...
scripts/      Standalone figure-generation script
...
`requirements.txt` lists loose version bounds; `requirements-lock.txt`
contains the exact environment used for the reported runs.
...
`scripts/make_figures.py` regenerates the manuscript figures from the result
tables in `results/`.
